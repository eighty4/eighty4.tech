...
export async function getRecentWriting(count: number = 10): Promise<Array<WritingContent>> {
...
export async function getProjects(category: ProjectCategory): Promiise<Array<Project>> {
    return (await getCollection('projects')).filter(p => p.data.category === category).sort(
